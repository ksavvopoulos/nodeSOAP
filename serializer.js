var Serializer = function () {
    ...
    */
    ...
    this.toXmlRetrieveMultiple = function (options) {
        ...
        if (options.ColumnSet && options.ColumnSet.length > 0) {
            columset = options.ColumnSet.map(function (c) {
                return "<b:string>" + c + "</b:string>";
            });

            xml += "<a:ColumnSet><a:AllColumns>false</a:AllColumns>"+
                "<a:Columns xmlns:b='http://schemas.microsoft.com/2003/10/Serialization/Arrays'>" +
                columset.join('') +
                "</a:Columns></a:ColumnSet>";
        } else {
            xml += "<a:ColumnSet><a:AllColumns>true</a:AllColumns></a:ColumnSet>";
        }

        if (options.Criteria) {
            if (options.Criteria.Conditions) {
                if (options.Criteria.Conditions.FilterOperators) {
                    filters = options.Criteria.Conditions.FilterOperators.map(function (c) {
                        return '<a:FilterOperator>' + c + '</a:FilterOperator>';
                    });
                    xml += '\n<a:Criteria><a:FilterOperator>' + filters.join('') + '</a:FilterOperator></a:Criteria>';
                }
            }
        }else{
            xml+="<a:Criteria><a:Conditions /><a:FilterOperator>And</a:FilterOperator>"+
        ...
        if (options.EntityName) {
            xml += "<a:EntityName>" + options.EntityName + "</a:EntityName>";
        }

        xml+="<a:LinkEntities /><a:Orders /><a:PageInfo><a:Count>0</a:Count>"+
            "<a:PageNumber>0</a:PageNumber><a:PagingCookie i:nil='true' />"+
            "<a:ReturnTotalRecordCount>false</a:ReturnTotalRecordCount>"+
            "</a:PageInfo><a:NoLock>false</a:NoLock>";
        return xml;
    };

    this.toXmlRetrieveAllEntities = function(options){
        var xml='';
        return xml;
    };
